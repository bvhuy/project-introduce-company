...
if(isPost()) {
    ...
    if(empty(trim($body['icon']))) {
        $errors['icon']['required'] = 'Icon bắt buộc phải nhập hoặc chọn';
    }
    ...
    if(empty($errors)) {
        ...
        if($insertStatus) {
            ...
            redirect('admin?module=services'); //Chuyển hướng sang trang danh sách nhóm người dùng
        } else {
            setFlashData('msg', 'Hệ thống đang gặp sự cố! Vui lòng thử lại sau.');
            setFlashData('msg_type', 'danger');
            redirect('admin?module=services&action=add'); //Load lại trang thêm người dùng
        }
    } else {
        //Có lỗi xảy ra
