...
//Lấy dữ liệu dịch vụ
$listServices = getRaw("SELECT services.id, name, slug, icon, services.create_at, user_id, fullname FROM services INNER JOIN users ON services.user_id = users.id $filter ORDER BY services.create_at DESC LIMIT $offset, $perPage");
...
?>
    ...
    <section class="content">
        <div class="container-fluid">
            <?php if($checkRoleAdd): ?>
            <a href="<?php echo getLinkAdmin('services', 'add'); ?>" class="btn btn-primary btn-sm"><i
                        class="fa fa-plus"></i> Thêm dịch vụ</a>
            <hr>
            <?php endif; ?>
            <form action="" method="get">
                <div class="row">
                    ...
                    <div class="col-6">
                        <input type="search" class="form-control" name="keyword" placeholder="Nhập tên dịch vụ..."
                                ...
                                if (!empty($listServices)):
                                    foreach ($listServices as $key => $item):
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo (isFontIcon($item['icon'])) ? html_entity_decode($item['icon']) : '<img width="80" src="' . $item['icon'] . '" />'; ?></td>
                                            <td>
                                                <a href="<?php echo getLinkAdmin('services', 'edit', ['id' => $item['id']]); ?>"><?php echo $item['name']; ?></a>
                                                <?php if($checkRoleDuplicate): ?>
                                                <a href="<?php echo getLinkAdmin('services', 'duplicate', ['id' => $item['id']]); ?>"
                                            ...
                                            <?php if($checkRoleDelete): ?>
                                            <td class="text-center">
                                                <a href="<?php echo getLinkAdmin('services', 'delete', ['id' => $item['id']]); ?>"
                            ...
                            <nav aria-label="Page navigation example" class="d-flex justify-content-end">
                                <ul class="pagination pagination-sm">
                                    ...
                                    for ($index = $begin; $index <= $end; $index++) {
                                        ...
                                        echo '<li class="page-item' . $active . '"><a class="page-link" href="' . _WEB_HOST_ROOT_ADMIN . '?module=services' . $queryString . '&page=' . $index . '">' . $index . '</a></li>';
                                    }
                                    if ($page < $maxPage) {
                                        $nextPage = $page + 1;
