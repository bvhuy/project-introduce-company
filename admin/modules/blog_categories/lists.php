...
$listCate = getRaw("SELECT blog_categories.id, name, blog_categories.slug, blog_categories.create_at, user_id, fullname, (SELECT count(blogs.id) FROM blogs WHERE blogs.category_id = blog_categories.id) as blogs_count FROM blog_categories INNER JOIN users ON blog_categories.user_id = users.id $filter ORDER BY blog_categories.create_at DESC LIMIT $offset, $perPage");
                                        ...
                                        if (!empty($listCate)):
                                            foreach ($listCate as $key => $item):
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="<?php echo getLinkAdmin('blog_categories', '', ['id' => $item['id'], 'view' => 'edit']); ?>"><?php echo $item['name']; ?></a> <?php echo '(' . $item['blogs_count'] . ')'; ?>
                                                        <?php if($checkRoleDuplicate): ?>
                                                        <a href="<?php echo getLinkAdmin('blog_categories', 'duplicate', ['id' => $item['id']]); ?>"
                                                           style="padding: 0 5px;" class="btn btn-danger btn-sm">Nhân
                                                            bản</a>
                                                        <?php endif; ?>
                                                        <a target="_blank" href="<?php echo getLinkModule('blog_categories', $item['id']); ?>" style="padding: 0 5px;"
                                                    ...
                                                    <?php if($checkRoleDelete): ?>
                                                    <td class="text-center">
                                                        <a href="<?php echo getLinkAdmin('blog_categories', 'delete', ['id' => $item['id']]); ?>"
                                    ...
                                    <nav aria-label="Page navigation example" class="d-flex justify-content-end">
                                        <ul class="pagination pagination-sm">
                                            ...
                                            for ($index = $begin; $index <= $end; $index++) {
                                                ...
                                                echo '<li class="page-item' . $active . '"><a class="page-link" href="' . _WEB_HOST_ROOT_ADMIN . '?module=blog_categories' . $queryString . '&page=' . $index . '">' . $index . '</a></li>';
                                            }
                                            if ($page < $maxPage) {
                                                $nextPage = $page + 1;
